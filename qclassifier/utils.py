...
def find_best_initial_conditions(model, data_train, epochs = 5, batch_size = 20, samples = 20, lr = 1e-2):
    ...
    for n in pbar:
        ...
        result = train(model, optimizer, data_train, data_train, epochs = epochs, batch_size = batch_size, test_accuracy = False, show_progress = False)
        accuracy = result['Train accuracy'][-1]
        ...
        pbar.set_postfix({'Lowest loss' : least_loss})

    return best_model


def train(model, optimizer, data_train, data_test, epochs = 500, batch_size = 20, test_accuracy = False, patience = 0, show_progress = True):
    ...
    for e in pbar:
        ...
        if patience > 0:
            ...
            # Se è migliorato salvo il modello e resetto il resto
            # if test_accuracy:   a = accuracy_test_history[e]
            # else:               a = accuracy_train_history[e]
            # if a > best_accuracy:
            #     best_accuracy = a
            #     best_model = copy.deepcopy(model)
            #     epochs_no_improvement = 0

            if test_accuracy: err = loss_test_history[e]
            else:             err = loss_train_history[e]
...
def test_accuracy(model, data, batch_size = 20):
    ...
    return torch.sum(Y_pred == Y_true) / N


def convert_cartesian_to_bloch(p):
...
def convert_state_to_cartesian(psi):

    rho = torch.einsum('bi,bj -> bji',torch.conj(psi),psi)
   
    return convert_density_to_cartesian(rho)


def convert_density_to_cartesian(rho):
