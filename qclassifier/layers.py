...
class QLayerOriginal(nn.Module):
    ...
    def forward(self, rho, x, noise_channels = []):
        ...
        # Angoli generalizzati (batch_size x D)
        # Tengo conto che se D non è un multiplo di 3 devo aggiungere degli angoli nulli
        phi = torch.zeros((batch_size,self.D3), device = self.device)
        phi = torch.tile(self.theta,(batch_size,1))
        phi[:,0:self.D] = phi[:,0:self.D] + torch.einsum('j,bj -> bj', self.w, x)

        # Creo D//3 unitari differenti e li applico
        for i in range(self.D3//3):

            U = torch.zeros((batch_size,2,2), dtype = torch.cfloat, device = self.device)
            
            U[:,0,0] =  torch.cos(phi[:,i*3]/2)*torch.exp( 1j * (phi[:,i*3+1] + phi[:,i*3+2])/2) 
            U[:,0,1] = -torch.sin(phi[:,i*3]/2)*torch.exp( 1j * (phi[:,i*3+1] - phi[:,i*3+2])/2)
            U[:,1,0] =  torch.sin(phi[:,i*3]/2)*torch.exp(-1j * (phi[:,i*3+1] - phi[:,i*3+2])/2)
            U[:,1,1] =  torch.cos(phi[:,i*3]/2)*torch.exp(-1j * (phi[:,i*3+1] + phi[:,i*3+2])/2)
...
# Sono necessari 3 * (D + 1) parametri a livello. La funzione di loss è comunque
# definita come nel modello precedente, quindi lavoro su un solo qubit
...
class QLayer(nn.Module):
    ...
    def forward(self, rho, x):
        ...
        U[:,0,0] =  torch.cos(phi[:,0]/2)*torch.exp( 1j * (phi[:,1] + phi[:,2])/2) 
        U[:,0,1] = -torch.sin(phi[:,0]/2)*torch.exp( 1j * (phi[:,1] - phi[:,2])/2)
        U[:,1,0] =  torch.sin(phi[:,0]/2)*torch.exp(-1j * (phi[:,1] - phi[:,2])/2)
        U[:,1,1] =  torch.cos(phi[:,0]/2)*torch.exp(-1j * (phi[:,1] + phi[:,2])/2)
...
# futuro si potrebbe implementare l'entanglement. Sono necessari 3 * C * (D + 1)
# parametri a livello.
...
class QLayerMultiQubit(nn.Module):
    ...
    def forward(self, rho, x):
        ...
        U = torch.zeros((batch_size,self.C,2,2), dtype = torch.cfloat, device = self.device)
