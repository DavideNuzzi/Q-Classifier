...
from . import layers
from .utils import convert_cartesian_to_bloch
...
class SingeQubitClassifier(nn.Module):
    ...
    def loss(self, rho, labels):
        ...
        # Calcolo per ogni label del batch, la fidelity attesa rispetto a tutti gli stati
    ...
    def get_fidelity(self, rho):
        return torch.real(torch.einsum('c,ic,bij,jc -> bc', self.alpha, torch.conj(self.psi_c), rho, self.psi_c) )

    def get_expected_fidelity(self,labels):

        psi_l = self.psi_c[:,labels]
        return torch.square(torch.abs(torch.einsum('ic,ib -> bc', torch.conj(self.psi_c), psi_l) ))

    def get_maximally_orthogonal_states(self):

        # Base computazionale
        if self.C == 2:
            psi_c = torch.tensor([[1,0],[0,1]], dtype = torch.cfloat, device = self.device)
        ...
        if self.C == 4:
            psi_c = torch.tensor([  [1,   1/np.sqrt(3),      1/np.sqrt(3),                              1/np.sqrt(3) ],
                                    [0,   np.sqrt(2/3),      np.sqrt(2/3) * np.exp(1j * 2 * np.pi/3),   np.sqrt(2/3) * np.exp(-1j * 2 * np.pi/3)    ]], 
                                    dtype = torch.cfloat, device = self.device)   
        ...
        if self.C == 5:
            ...
            psi_c[:,0] =  convert_cartesian_to_bloch((0,0,1))
            psi_c[:,1] =  convert_cartesian_to_bloch((0,0,-1))
            psi_c[:,2] =  convert_cartesian_to_bloch((1,0,0))
            psi_c[:,3] =  convert_cartesian_to_bloch((-1/2,np.sqrt(3)/2,0))
            psi_c[:,4] =  convert_cartesian_to_bloch((-1/2,-np.sqrt(3)/2,0))
        ...
        if self.C == 10:
            psi_c = torch.zeros((2,10),dtype = torch.cfloat, device = self.device)

            a = np.sqrt(2) / 2 
            b = np.power(8,1/4)/4

            psi_c[:,0] = convert_cartesian_to_bloch((0,0,a+b))
            psi_c[:,1] = convert_cartesian_to_bloch((0,0,-a-b))
            psi_c[:,2] = convert_cartesian_to_bloch((1/2,1/2,b))
            psi_c[:,3] = convert_cartesian_to_bloch((1/2,-1/2,b))
            psi_c[:,4] = convert_cartesian_to_bloch((-1/2,1/2,b))
            psi_c[:,5] = convert_cartesian_to_bloch((-1/2,-1/2,b))
            psi_c[:,6] = convert_cartesian_to_bloch((0,a,-b))
            psi_c[:,7] = convert_cartesian_to_bloch((0,-a,-b))
            psi_c[:,8] = convert_cartesian_to_bloch((a,0,-b))
            psi_c[:,9] = convert_cartesian_to_bloch((-a,0,-b))
...
class ClassifierOriginal(SingeQubitClassifier):
    ...
    def __init__(self, D = 3, L = 1, C = 2, device = 'cpu', noise_channels = []):
        ...
        for _ in range(L):  layers.append(QLayerOriginal(self.D, device = device))
        self.layers = layers
    ...
    def forward(self, x):
        ...
        rho = torch.zeros((batch_size, 2, 2), dtype = torch.cfloat, device = self.device)
        rho[:,0,0] = 1
        ...
        for i in range(self.L):
            ...
            # Per come funziona questo modello devo passare i canali di rumore a
            # ogni livello in modo che vengano applicati dopo ogni unitario
...
class ClassifierImproved(SingeQubitClassifier):
    ...
    def __init__(self, D = 3, L = 1, C = 2, device = 'cpu', noise_channels = []):
        ...
        layers = nn.ModuleList()
        for _ in range(L):  layers.append(QLayer(self.D, device = device))
        self.layers = layers
...
class ClassifierMultiQubit(nn.Module):
    ...
    def forward(self, x):
        ...
        # Costruisco uno stato per ogni qubit (uno per classe per sample del batch)
        rho = torch.zeros((batch_size, self.C, 2, 2), dtype = torch.cfloat, device = self.device)
        rho[:,:,0,0] = 1
    ...
    def loss(self, rho, labels):
        ...
        # Le fidelity attese sono banalmente tutte zero tranne una sola, quella corrispondente alla classe giusta
        # Y_c   = (batch_size x C)
